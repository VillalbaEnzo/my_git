...
tests_run: all $(TEST_OBJ)
	$(CC) -o $(TEST_BIN) $(TEMP_SRC) $(TEST_OBJ) -Iinclude --coverage -lcriterion
	-./$(TEST_BIN)

coverage: tests_run
	gcovr --gcov-executable "llvm-cov gcov" --exclude tests/

.PHONY: all clean fclean re vgrind tests_run coverage
