...
import { FileText, ShieldAlert, AlertTriangle, UploadCloud, Bell, BellRing } from 'lucide-react'
...
export default function Profile() {
  ...
  async function uploadDocuments() {
    setUploadError(null)
    if (!cniRecto || !cniVerso || !attestationScolarite) {
      setUploadError('Les 3 documents (recto, verso, attestation) sont requis.')
      return
    }
    setUploading(true)
    try {
      const fd = new FormData()
      fd.append('cniRecto', cniRecto)
      fd.append('cniVerso', cniVerso)
      fd.append('attestationScolarite', attestationScolarite)
      const { data } = await api.post<{ cniRectoUrl: string; cniVersoUrl: string; attestationScolariteUrl: string; statutInscription: string }>(
        '/auth/me/document', fd, { headers: { 'Content-Type': undefined } },
      )
      setUser(prev => prev ? { ...prev, ...data } : prev)
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error
      setUploadError(typeof msg === 'string' ? msg : "Erreur lors de l'envoi des documents.")
    } finally {
      setUploading(false)
    }
  }
  ...
  return (
    <div className="grain relative min-h-screen overflow-hidden bg-cream dark:bg-ink-950">
      ...
      <main className="relative mx-auto w-full max-w-xl px-6 pb-16">
        ...
        {user.cniRectoUrl && user.cniVersoUrl && user.attestationScolariteUrl ? (
          <div className="flex flex-col gap-2 rounded-2xl border border-stone-200 bg-white p-5 dark:border-ink-800 dark:bg-ink-900">
            {([
              ['CNI — recto', user.cniRectoUrl],
              ['CNI — verso', user.cniVersoUrl],
              ['Attestation de scolarité', user.attestationScolariteUrl],
            ] as const).map(([docLabel, url]) => (
              <div key={docLabel} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <FileText size={18} className="text-primary-400" />
                  <span className="text-sm font-medium text-ink-800 dark:text-stone-200">{docLabel}</span>
                </div>
                <button
          ...
          <div className="flex flex-col gap-4 rounded-2xl border-2 border-dashed border-primary/40 bg-primary/[0.04] p-6">
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm text-stone-600 dark:text-stone-300">
                Les 3 documents (recto, verso, attestation) sont requis pour finaliser votre dossier.
              </p>
            </div>

            <FileField
              id="profileCniRecto" label="CNI — recto" file={cniRecto} onChange={setCniRecto}
              accept=".jpg,.jpeg,.png,.webp" hint="JPG, PNG ou WEBP — 5 Mo max"
            />
            <FileField
              id="profileCniVerso" label="CNI — verso" file={cniVerso} onChange={setCniVerso}
              accept=".jpg,.jpeg,.png,.webp" hint="JPG, PNG ou WEBP — 5 Mo max"
            />
            <FileField
              id="profileAttestation" label="Attestation de scolarité" file={attestationScolarite} onChange={setAttestationScolarite}
              accept=".pdf,.jpg,.jpeg,.png" hint="PDF, JPG ou PNG — 5 Mo max"
            />

            {uploadError && (
              <p role="alert" className="text-sm font-medium text-red-500">{uploadError}</p>
            )}

            <button
              onClick={uploadDocuments}
              disabled={uploading}
              className="rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-white transition-colors hover:bg-cobalt-500 disabled:cursor-not-allowed disabled:opacity-60"
            >
