...
import { Search, MapPin, X, Store, Tag } from 'lucide-react'
import type { Categorie, CommerceWithDetails, OffreWithCommerce } from '../../types/commerce'
import MerchantCard from '../../components/MerchantCard/MerchantCard'
import OfferCard from '../../components/OfferCard/OfferCard'
import BottomNav from '../../components/BottomNav/BottomNav'
import { MerchantCardSkeleton, OfferCardSkeleton } from '../../components/ui/Skeleton'
import { useFavoris } from '../../hooks/useFavoris'
import api from '../../api/client'
import { cn } from '../../lib/utils'

type View = 'commerces' | 'offres'

export default function Annuaire() {
  const [view, setView] = useState<View>('offres')
  const [commerces, setCommerces] = useState<CommerceWithDetails[]>([])
  const [offres, setOffres] = useState<OffreWithCommerce[]>([])
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<OffreWithCommerce[]>('/offres'),
    ])
      .then(([resC, resCat, resO]) => {
  ...
  const filteredOffres = useMemo(() => {
    ...
    return offres.filter(
      (o) =>
        (selectedCat === null || o.commerce.categorie.id === selectedCat) &&
        (q === '' ||
          o.titre.toLowerCase().includes(q) ||
          o.commerce.nom.toLowerCase().includes(q) ||
          o.description?.toLowerCase().includes(q)),
    )
  }, [offres, search, selectedCat])

  const chip = 'flex-shrink-0 whitespace-nowrap rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-colors'

  const count = view === 'commerces' ? filteredCommerces.length : filteredOffres.length
  const countLabel = view === 'commerces'
    ? `${count} commerce${count > 1 ? 's' : ''}`
    ...
    <div className="grain relative min-h-screen overflow-hidden bg-cream pb-28 dark:bg-ink-950">
      ...
      <header className="relative px-6 pb-5 pt-12">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <button
            ...
            className={cn(
              'flex flex-col items-center gap-1.5 rounded-2xl border-2 py-4 font-display font-bold transition-all duration-200',
              view === 'offres'
                ? 'border-cobalt-500 bg-cobalt-500 text-white shadow-cobalt'
                : 'border-stone-200 bg-white text-stone-500 dark:border-white/10 dark:bg-ink-900 dark:text-stone-400',
            )}
          >
        ...
        <div className="relative mt-5 flex items-center">
          ...
          <input
            ...
            placeholder={view === 'commerces' ? 'Rechercher un commerce…' : 'Rechercher une offre ou un commerce…'}
            value={search}
      ...
      <div className="px-6 pb-1 pt-4">
        <span className="tnum text-xs font-semibold uppercase tracking-wider text-stone-400">
          {loading ? 'Chargement…' : error ? error : countLabel}
        </span>
      </div>

      <div className="flex flex-col gap-3 px-6 pb-4 pt-2">
        {loading && view === 'commerces' &&
        ...
        {!loading && !error && view === 'commerces' && filteredCommerces.length === 0 && (
        ...
        {!loading && !error && view === 'offres' && filteredOffres.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 text-stone-400">
            <Tag size={32} strokeWidth={1.5} className="text-cobalt-500" />
            <p className="text-sm">Aucune offre active pour le moment.</p>
          </div>
        )}
        {!loading && !error && view === 'offres' &&
          filteredOffres.map((offre, i) => (
