...
export default function MerchantCard({ commerce, isFavori = false, onToggleFavori }: Props) {
  ...
  return (
    <motion.div
      ...
      onClick={() => navigate(`/annuaire/${commerce.id}`)}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => { if (e.key === 'Enter') navigate(`/annuaire/${commerce.id}`) }}
      className="lift group relative cursor-pointer overflow-hidden rounded-2xl border border-black/[0.06] bg-white p-5 hover:border-cobalt-500/40 dark:border-white/[0.07] dark:bg-ink-900"
    >
