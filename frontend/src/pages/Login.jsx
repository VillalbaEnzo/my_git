...
import { Scissors, AlertCircle } from 'lucide-react'; // Ajout de l'icône d'alerte
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      setPassword(''); // On vide le mot de passe par sécurité/confort
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-neutral-900 px-4">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden animate-fade-in">
        <div className="p-8">
          ...
          <h2 className="text-2xl font-semibold text-center text-neutral-900 mb-1">L'Atelier Coiffure</h2>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {/* --- ZONE D'ERREUR --- */}
            {error && (
              <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded text-sm flex items-center gap-2 animate-[shake_0.5s]">
                <AlertCircle size={18} />
                <span>{error}</span>
              </div>
            )}
            ...
            <div>
              ...
              <input 
                ...
                value={email} 
                onChange={e => { setEmail(e.target.value); setError(''); }} // Efface l'erreur quand on tape
              />
            </div>
        ...
        <div className="bg-neutral-50 px-8 py-4 border-t border-neutral-100 text-center">
          <p className="text-xs text-neutral-400">© 2026 Salon Pacheco</p>
        </div>
      </div>
    </div>
  );
