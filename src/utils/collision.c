...
static bool is_in_tower_area(sprite_list_t *plane, sprite_list_t *all_entities)
{
    ...
    while (tmp) {
        if (!tmp->is_plane) {
            sfVector2f tower_pos = tmp->start_pos;
...
static void check_intersects(sfFloatRect area, sfFloatRect other_box,
...
static void check_collision_for_entity(game_t *game, quadtree_t *qt,
    sprite_list_t *plane)
{
