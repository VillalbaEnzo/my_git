...
static int get_condition_len(char *token, int *count, int *parenthesis)
{
...
static char **extract_condition_tokens(char **tokens, int *last_pos)
{
    ...
    for (int i = start; tokens[i]; i++) {
        if (get_condition_len(tokens[i], &count, &parenthesis))
            break;
    }
...
static int get_body_len(char *token, int *count, int *end)
{
    if (my_strcmp(token, "while") == 0 || my_strcmp(token, "foreach") == 0)
        (*end)++;
    if (my_strcmp(token, "end") == 0) {
        (*end)--;
        if ((*end) == 0) {
            (*count)++;
            return 1;
...
char **extract_body_tokens(char **tokens, int *last_pos)
{
    ...
    for (int i = start; tokens[i]; i++) {
        if (get_body_len(tokens[i], &count, &end))
            break;
    }
