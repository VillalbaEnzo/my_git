...
{
    sfFloatRect plane_rect = sfRectangleShape_getGlobalBounds(plane->hitbox);
    int index = -1;
    // int i = 0; // (Variable inutile ici, vous pouvez l'enlever)
    ...
    // --- SÉCURITÉ ANTI-CRASH ---
    if (node->nbr_plane >= QT_NODE_CAPACITY)
        return; // Le nœud est plein à craquer, on arrête pour éviter le Segfault
    // ---------------------------

    node->plane[node->nbr_plane] = plane;
    node->nbr_plane++;

    // On garde 'CAPACITY' ici pour savoir quand diviser
