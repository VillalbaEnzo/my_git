...
static void update_save_btn_state(game_t *game, sfVector2f *mouse_pos)
{
    sfText *text = game->pl_ui->save_btn;
    sfFloatRect bounds = sfText_getGlobalBounds(text);
...
void update_other_btn(game_t *game)
{
    ...
    update_save_btn_state(game, &mouse_pos);
}
