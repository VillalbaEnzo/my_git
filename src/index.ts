let liveStreamRes: http.ServerResponse | null = null;
let liveStreamHeader: Buffer | null = null;
...
const proxyServer = http.createServer((req, res) => {
    ...
    if (req.url === '/live-stream') {
        // === NOUVEAU : Répondre au Chromecast s'il vérifie la sécurité (CORS) ===
        ...
        console.log('📺 Chromecast connecté au flux live !');
        res.writeHead(200, {
            'Content-Type': 'video/webm',
            // On enlève le chunked, on laisse la connexion ouverte simplement
        ...
        if (liveStreamHeader) {
            liveStreamRes.write(liveStreamHeader);
        }

        req.on('close', () => {
            console.log('🔌 Chromecast déconnecté du flux live.');
            if (liveStreamRes === res) liveStreamRes = null;
        });
        return;
    }
...
wss.on('connection', (ws: WebSocket) => {
	ws.on('message', async (message: Buffer, isBinary: boolean) => {
        if (isBinary) {
            // NOUVEAU : Affiche une boîte dans le terminal pour prouver que la vidéo arrive !
            process.stdout.write('📦'); 
            
            if (!liveStreamHeader) liveStreamHeader = message;
            if (liveStreamRes) liveStreamRes.write(message);
            return;
        }
		...
		try {
			...
			// MODIFIÉ : On utilise le deviceId envoyé par l'extension avec les cookies
				...
				if (targetDevice) {
					try {
						if (data.castType === "LIVE_STREAM") {
                            console.log(`➡️ Envoi du DIRECT WEB vers : ${targetDevice.name}...`);
                            liveStreamHeader = null; 
                            
                            const media = {
                                url: `http://${LOCAL_IP}:${PROXY_PORT}/live-stream`,
                                mimeType: 'video/webm',
                                title: data.media.title || 'Page Web en direct',
                                streamType: 'LIVE' as const // 👈 NOUVEAU : On prévient la télé que c'est du direct !
                            };
