...
static void destroy_load(game_t *game)
{
    if (game->start_menu->input_box)
        sfRectangleShape_destroy(game->start_menu->input_box);
    if (game->start_menu->load_input)
        sfText_destroy(game->start_menu->load_input);
    if (game->start_menu->input)
        free(game->start_menu->input);
}

static void free_start_menu(game_t *game)
{
...
static void destroy_textures(game_t *game)
{
    if (!game->textures)
        return;
    if (game->textures->dirt_text)
        sfTexture_destroy(game->textures->dirt_text);
    if (game->textures->grass_text)
        sfTexture_destroy(game->textures->grass_text);
    if (game->textures->snow_text)
        sfTexture_destroy(game->textures->snow_text);
    if (game->textures->stone_text)
        sfTexture_destroy(game->textures->stone_text);
    if (game->textures->secret_text)
        sfTexture_destroy(game->textures->secret_text);
    free(game->textures);
}
