...
static bool create_plane_requirement(sprite_list_t *plane, game_t *game)
{
    plane->texture = game->plane_texture;
    if (!plane->texture)
        return false;
...
static bool create_tower_requirement(sprite_list_t *tower, game_t *game)
{
    tower->texture = game->tower_texture;
    if (!tower->texture)
        return false;
...
static bool create_plane(sprite_list_t *plane, game_t *game)
{
    sfVector2u size;

    if (!create_plane_requirement(plane, game))
        return false;
...
static bool create_tower(sprite_list_t *tower, game_t *game)
{
    sfVector2u size;

    if (!create_tower_requirement(tower, game))
        return false;
...
void load_entities_resources(game_t *game)
{
    ...
    while (current != NULL) {
        if (current->is_plane)
            create_plane(current, game);
        if (!current->is_plane)
            create_tower(current, game);
