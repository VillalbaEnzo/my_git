...
bool create_start_btn(game_t *game)
{
    ...
    sfVector2f pos = {810, 385};

    game->start_menu->start_btn->rect = sfRectangleShape_create();
    if (!game->start_menu->start_btn->rect)
        return false;
    sfRectangleShape_setSize(game->start_menu->start_btn->rect, size);
    sfRectangleShape_setPosition(game->start_menu->start_btn->rect, pos);
    ...
    sfRectangleShape_setOutlineColor(game->start_menu->start_btn->rect,
        sfGreen);
    return true;
}
