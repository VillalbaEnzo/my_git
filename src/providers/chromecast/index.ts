...
export class ChromecastProvider implements ICastProvider {
    ...
    cast(media: MediaPayload, ip: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            } else if (media.streamType === 'LIVE') {
                // 👈 NOUVEAU : On garde l'URL intacte pour le flux en direct !
                console.log(`📡 Utilisation de l'URL directe pour le flux LIVE`);
            } else if (media.url.startsWith('http://') || media.url.startsWith('https://')) {
                // ⚠️ J'ai aussi remplacé 127.0.0.1 par ton IP ici, sinon tes vidéos classiques planteront aussi !
                contentId = `http://192.168.1.208:8081/${encodeURIComponent(media.url)}`;
            ...
            client.connect(ip, () => {
                ...
                client.launch(DefaultMediaReceiver, (err: any, player: any) => {
                    ...
                    const mediaInfo = {
                        ...
                        streamType: media.streamType || 'BUFFERED', // 👈 MODIFIÉ
