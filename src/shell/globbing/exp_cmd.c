...
static char *get_sub_input(FILE *c_output)
{
    ...
    size_t thrash = 1;
    int f_char = fgetc(c_output);

    if (f_char == EOF) {
        input[0] = '\0';
        return input;
    }
    getdelim(&input, &thrash, '\0', c_output);
    asprintf(&final_input, "%c%s", f_char, input);
    free(input);
    return remove_newlines(final_input);
}
