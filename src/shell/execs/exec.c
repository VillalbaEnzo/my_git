...
int child_code(ast_cmd_t *cmd_node, char **envp)
{
    ...
    if (checker == -1 && errno != EISDIR) {
        new_cmd = handle_path(CMD, envp, command_saver);
        free(CMD);
        CMD = new_cmd;
    }
    if (CMD == NULL)
...
static int fork_and_exec(ast_cmd_t *cmd_node, char **envp)
{
    pid_t cpid = fork();
...
int execute_command(ast_cmd_t *cmd_node, char **envp, data_t *data)
{
    ...
    if (cmd_node == NULL || cmd_node->arguments == NULL ||
        ...
        return 0;
    expand_aliases(cmd_node, data);
    if (args_exp(cmd_node, data) == 1)
        return 1;
    is_builtin = exec_builtin(cmd_node, envp, data);
    if (is_builtin != -1)
        return is_builtin;
    return fork_and_exec(cmd_node, envp);
}

static int next_switch(struct ast_node *root, data_t *data, char **envp)
{
    switch (root->type) {
        case AND:
...
int traverse_tree(struct ast_node *root, data_t *data, char **envp)
{
    ...
    switch (root->type) {
        ...
        default:
            return next_switch(root, data, envp);
    }
}
