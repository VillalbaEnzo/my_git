...
* **Steps:**
  ...
  2. Located a review for the "Banana Juice (1000ml)" product written by "Bender", revealing his email address: `bender@juice-sh.op`.
  3. Navigated to the login page and entered the discovered email followed by an SQL injection payload: `bender@juice-sh.op'--`.
  4. Submitted the form with a dummy password.
  5. Observation: The `'--` sequence successfully commented out the remainder of the server's SQL query (the password check logic), allowing an immediate login to Bender's account without a valid password.
