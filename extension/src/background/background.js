...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "GET_DEVICES") {
    ...
    else if (request.action === "CHECK_MEDIA") {
        chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
            const tabId = tabs[0].id;
            const tabUrl = tabs[0].url;

            // 1. Vérifier dans le scrapper réseau
            if (detectedMedia[tabId]) {
                sendResponse({ hasVideo: true, pageUrl: tabUrl });
                return;
            }

            // 2. Vérifier dans le DOM HTML (fallback)
            try {
                ...
                if (videoData && videoData.url) {
                    ...
                    sendResponse({ hasVideo: false, pageUrl: tabUrl });
                }
            } catch (err) { 
                sendResponse({ hasVideo: false, pageUrl: tabUrl }); 
            }
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
            ...
            if (request.type === "PAGE") {
                const pageData = { url: request.url, title: tab.title, mimeType: 'text/html' };
                socket.send(JSON.stringify({ type: "CAST_REQUEST", castType: "PAGE", deviceId: request.deviceId, media: pageData }));
                sendResponse({ success: true, media: pageData });
                return;
            }
            let cookies = {};
            try {
            ...
            if (detectedMedia[tabId]) {
                ...
                socket.send(JSON.stringify({ type: "CAST_REQUEST", deviceId: request.deviceId, media: videoData, castType: 'VIDEO' }));
            ...
            try {
                ...
                if (videoData && videoData.url) {
                    ...
                    socket.send(JSON.stringify({ type: "CAST_REQUEST", castType: 'VIDEO', deviceId: request.deviceId, media: videoData }));
                    sendResponse({ success: true, media: videoData });
